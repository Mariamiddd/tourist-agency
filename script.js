...
const notify = {
    success(msg) {
        Swal.fire({ title: 'Success!', text: msg, icon: 'success', confirmButtonColor: '#005A9C' });
    },
    error(msg) {
    ...
    signUpForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setTimeout(() => document.getElementById('popupOverlay').style.display = 'none', 2000);
            } else {
...
function openSignInPopup() {
    ...
    document.getElementById('closeSignIn').onclick = () => signinoverlay.style.display = 'none';
...
async function updateAuthUI() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            else userNameSpan.innerText = "TRAVELER";
        }
    } catch (error) {
    ...
    try {
        ...
        if (response.ok && data.access_token) {
            if (email === "admin@gmail.com") {
                ...
                notify.error("You are not authorized to access the admin dashboard.");
            }
        } else {
...
async function submitQuestion() {
    ...
    try {
        ...
        notify.error("Failed to submit question.");
    }
}
...
async function deleteQuestion(id) {
    notify.confirm(
        "Delete permanently?",
        "This will remove the question from the database.",
        async () => {
            try {
                const res = await fetch(`${QUESTION_API}/${id}`, { method: 'DELETE' });
                if (res.ok) {
                    loadAdminQuestions();
                    Swal.fire({
                        toast: true,
                ...
                notify.error("Server error. Could not delete.");
            }
        }
    );
}

// Destination & Weather Functions
...
function renderDestinations(data) {
    ...
    data.forEach(dest => {
        ...
        destDiv.innerHTML = `
            <div class="fav-container" style="position: absolute; right: 10px; top: 10px;"></div>
            ...
            <div class="card-body">
                ...
                <span class="fav-btn ${dest.isFeatured ? 'active' : ''}" data-id="${dest.id}" data-featured="${dest.isFeatured}">
...
function handleFavoriteToggle(favBtn) {
    ...
    favBtn.setAttribute('data-featured', newStatus.toString());
...
async function updateFavoriteQuietly(id, newStatus) {
    try {
        ...
        if (!response.ok) {
            fetchDestinations(); // Re-fetch to correct state on failure
...
function handleAddToCart(button) {
    ...
    const price = parseFloat(mainCard.querySelector('.price').textContent.replace(/[^0-9.]/g, ''));
    const cartBox = document.getElementById('cart-items-container');
    const existingItem = Array.from(cartBox.querySelectorAll('.cart-item')).find(row => row.querySelector('h3').textContent === title);
...
function handleCheckout() {
    ...
    if (cartBox.children.length === 0 || cartBox.innerText.includes('empty')) {
        notify.error("Your cart is empty! Choose a destination first.");
        return;
    }
    ...
    setTimeout(() => {
        notify.success("Your Greek Odyssey has been booked successfully.");
...
window.addEventListener('load', () => {
    ...
    // if its admin, show dashboard
    if (sessionStorage.getItem('admin_token')) {
        showDashboard();
    }

    // Q&A page loading if exists
...
document.addEventListener('click', (e) => {
    ...
    if (nav && nav.classList.contains('toggle') && !nav.contains(e.target) && !hamburger.contains(e.target)) {
        nav.classList.remove('toggle');
    }
    // Close cart
    if (cartContainer && !cartContainer.contains(e.target) && basketBtn && !basketBtn.contains(e.target) && !e.target.classList.contains('remove-item')) {
        cartContainer.classList.remove('active');
    }
});
...
if (searchInput) {
    searchInput.addEventListener('input', function() {
        ...
        Array.from(container.getElementsByClassName('destination-item')).forEach(item => {
        ...
        if (matchedMarker && filter.length > 2) {
            myMap.flyTo(matchedMarker.layer.getLatLng(), 8);
            matchedMarker.layer.openPopup();
        } else if (filter.length === 0) {
            myMap.setView([38.5, 23.5], 6);
        }
    });
}
